                    ...
                    <div class="">
                       ...
                       <div class="panel panel-primary">
                          ...
                          <div class="panel-body">
                                <form class="form" onsubmit="form_validate()" action="{{ url('admin/purchase-return/payment/'.$singlePurchaseReturn->purchase_return_master_id.'/update') }}" method="post">
                                    {{ method_field('PUT') }}
                                    {{ csrf_field() }}
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <table class="table-bordered table-striped" style="width: 100%">
                                            <tr>
                                                <td>Voucher Ref:</td><td><strong>{{ $singlePurchaseReturn->voucher_ref }}</td>
                                            </tr>
                                            <tr>
                                                <td> Date:</td><td><strong>{{ $singlePurchaseReturn->date }}</td>
                                            </tr>
                                            <tr>
                                                <td>Customer:</td><td><strong>{{ $singlePurchaseReturn->sup_name }}@if(!empty($singlePurchaseReturn->mobile_no)),{{ $singlePurchaseReturn->mobile_no }}@endif</td>
                                            </tr>
                                            <tr>
                                                <td>Total:</td><td><strong>{{  number_format($singlePurchaseReturn->memo_total,2) }}</td>
                                            </tr>
                                            
                                            
                                            <tr>
                                                <td>User:</td><td><strong>@if(isset($received_by)){{ $received_by->name }}@endif</td>
                                            </tr>
                                        </table>
                                    ...
                                    <div class="col-md-8">
                                        <span class="text-red">Note:</span> <strong>{{  $singlePurchaseReturn->note }}</strong>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">

                                            <input type="hidden" value="{{ $singlePurchaseReturn->purchase_return_master_id }}" name="sales_return_master_id"/>
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            
                                            <input type="hidden" value="{{ $singlePurchaseReturn->sup_name }}" name="sup_name" id="" class="form-control" readonly/>
                                            <input type="hidden" value="{{ $singlePurchaseReturn->supplier_id }}" name="supplier_id"/>
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <input type="hidden" name="updated_at" value="{{  $singlePurchaseReturn->date }}" class="form-control" readonly/>
                                        </div>
                                    </div>
                                            ...
                                            <table class="table-bordered" id="" width="100%">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>{{ number_format( ($value->rate * $value->quantity),2)}}</td>
                                                     <!--   <td><a href="#" class="btn btn-primary"><i class="fa fa-reply"></i> Return</a></td>-->
                                                     <!--    @if(Auth::user()->role == 'admin')
                                                        <td><a title="Edit" href="{{ url('/purchase/invoice/item-unit/'.$value->purchase_return_master_id."/".$value->purchase_return_master_id.'/edit') }}"  class="btn btn-info btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></a></td>
                                                        @endif -->
                                                    </tr>
                                                    @endforeach
                                                </tbody>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <form action="{{ url('admin/purchase/'.$singlePurchaseReturn->purchase_return_master_id.'/edit/invoice/info') }}" method="post" name="" id="">
              {{ csrf_field() }}
              {{ method_field('PUT') }}
               <div class="form-group">
                    <label for="">Invoice Date</label>
                    <input type="hidden" name="created_at" value="{{$singlePurchaseReturn->date}}"/>
                    <input type="date" name="updated_at" value="{{$singlePurchaseReturn->date}}" class="form-control" required/>
                </div>

              <div class="form-group">
                <label for="">Customer</label>
                <input type="hidden" name="supplier_id_old" value="{{ $singlePurchaseReturn->supplier_id }}"/>
                <select name="customer_id" id="supplier_id" class="form-control"  required> 
                    <option value="">---Select---</option>
                    @foreach($suppliers as $supplier)
                        <option value="{{ $supplier->supplier_id }}" {{ $singlePurchaseReturn->supplier_id == $supplier->supplier_id? 'selected' : null }}>{{ $supplier->sup_name }}-{{$supplier->address}}</option>
                    @endforeach
                </select>
             </div>
             <div class="form-group">
                <label for="">Advanced Amount</label>
                <input type="hidden" name="advanced_amount_old" value="{{ $singlePurchaseReturn->advanced_amount }}"/>
                <input type="text" autocomplete="OFF" name="advanced_amount" class="form-control" value="{{$singlePurchaseReturn->advanced_amount}}">
             </div>
             <div class="form-group">
                <label for="">Note</label>
                <textarea name="reason" id="ckEditor" class="form-control input-sm">{{ $singlePurchaseReturn->note}}</textarea>
             </div>
             <div class="form-group">
...
<div class="modal fade" id="myModal_2" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <form action="{{ url('admin/purchase/'.$singlePurchaseReturn->purchase_return_master_id.'/add-item/') }}" method="post" name="add_item_form">
              {{ csrf_field() }}
